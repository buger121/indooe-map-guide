        ...
        map.on('mapClickNode', function(e){
            ...
            if (e.nodeType == 5 && that.startNavi == true) {
                ...
                if (that.clickCount == 0) {
                    ...
                    that.startPoint = {x: e.hitCoord.x, y:e.hitCoord.y, fnum:e.FloorNum}
                    that.clickCount++;
                } else if (that.clickCount == 1) {
                    ender.value = e.name;
                    that.endPoint = {x: e.hitCoord.x, y:e.hitCoord.y, fnum:e.FloorNum}
                    that.navi = that.drawRoutes(map, that.navi, that.startPoint, that.endPoint);
                    that.clickCount++;
                } else {
                    //第三次点击的时候进行下一个循环
                    starter.value = e.name;
